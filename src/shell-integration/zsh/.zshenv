...
# environment and should not be sourced manually (unlike blackbox-integration).
#
...
# Restore the original ZDOTDIR value if BLACKBOX_ZSH_ZDOTDIR is set.
# Otherwise, unset the ZDOTDIR that was set during shell injection.
if [[ -n "${BLACKBOX_ZSH_ZDOTDIR+X}" ]]; then
    'builtin' 'export' ZDOTDIR="$BLACKBOX_ZSH_ZDOTDIR"
    'builtin' 'unset' 'BLACKBOX_ZSH_ZDOTDIR'
else
...
# Use try-always to have the right error code.
{
    ...
    #
    # Source the user's .zshenv before sourcing blackbox-integration because the
    # former might set fpath and other things without which blackbox-integration
    # won't work.
    #
    # Use typeset in case we are in a function with warn_create_global in
    # effect. Unlikely but better safe than sorry.
    'builtin' 'typeset' _blackbox_file=${ZDOTDIR-$HOME}"/.zshenv"
    # Zsh ignores unreadable rc files. We do the same.
    # Zsh ignores rc files that are directories, and so does source.
    [[ ! -r "$_blackbox_file" ]] || 'builtin' 'source' '--' "$_blackbox_file"
} always {
    if [[ -o 'interactive' ]]; then
        # ${(%):-%x} is the path to the current file.
        # On top of it we add :A:h to get the directory.
        'builtin' 'typeset' _blackbox_file="${${(%):-%x}:A:h}"/blackbox-integration
        if [[ -r "$_blackbox_file" ]]; then
            'builtin' 'autoload' '-Uz' '--' "$_blackbox_file"
            "${_blackbox_file:t}"
            'builtin' 'unfunction' '--' "${_blackbox_file:t}"
        fi
    fi
    'builtin' 'unset' '_blackbox_file'
}
