...
/// The `help` command shows general help about Blackbox Terminal. Recognized as either
...
pub fn run(alloc: Allocator) !u8 {
    ...
    try stdout.writeAll(
        \\Usage: blackbox [+action] [options]
        \\
        \\Run the Blackbox Terminal emulator or a specific helper action.
        \\
        \\If no `+action` is specified, run the Blackbox Terminal emulator.
        ...
        \\`blackbox -e top` will run the `top` command inside the terminal.
        \\
        \\On macOS, launching the terminal emulator from the CLI is not
        \\supported and only actions are supported. Use `open -na Blackbox.app`
        \\instead, or `open -na Blackbox.app --args --foo=bar --baz=quz` to pass
        \\arguments.
        \\
        \\Available actions:
        \\
