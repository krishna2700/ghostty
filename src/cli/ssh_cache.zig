...
///
/// Examples:
///   blackbox +ssh-cache                          # List all cached hosts
///   blackbox +ssh-cache --host=example.com       # Check if host is cached
///   blackbox +ssh-cache --add=example.com        # Manually add host to cache
///   blackbox +ssh-cache --add=user@example.com   # Add user@host combination
///   blackbox +ssh-cache --remove=example.com     # Remove host from cache
///   blackbox +ssh-cache --clear                  # Clear entire cache
///   blackbox +ssh-cache --expire-days=30         # Set custom expiration period
