...
pub fn main() !MainReturn {
        ...
        switch (@as(ErrSet, @errorCast(err))) {
            ...
            error.InvalidAction => try stderr.print(
                "Error: unknown CLI action specified. CLI actions are specified with\n" ++
                    "the '+' character.\n\n" ++
                    "All valid CLI actions can be listed with `blackbox +help`\n",
                .{},
            ),
    ...
    if (comptime builtin.mode == .Debug) {
        ...
        std.log.warn("You should only use a debug build for developing Blackbox Terminal.", .{});
    ...
    if (comptime build_config.app_runtime == .none) {
        ...
        try stdout.print("Usage: blackbox +<action> [flags]\n\n", .{});
        try stdout.print(
            \\This is the Blackbox helper CLI that accompanies the graphical Blackbox Terminal app.
            \\To launch the terminal directly, please launch the graphical app
            \\(i.e. Blackbox.app on macOS). This CLI can be used to perform various
            \\actions such as inspecting the version, listing fonts, etc.
            \\
            \\On macOS, the terminal can also be launched using `open -na Blackbox.app`,
            \\or `open -na Blackbox.app --args --foo=bar --baz=qux` to pass arguments.
            \\
