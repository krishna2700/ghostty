...
enum UpdateState: Equatable {
    ...
    struct UpdateAvailable {
        ...
        var releaseNotes: ReleaseNotes? {
            let currentCommit = Bundle.main.infoDictionary?["BlackboxCommit"] as? String
    ...
    enum ReleaseNotes {
        ...
        init?(displayVersionString: String, currentCommit: String?) {
            ...
            if let semver = Self.extractSemanticVersion(from: version) {
                ...
                if let url = URL(string: "https://blackbox.ai/docs/install/release-notes/\(slug)") {
                    self = .tagged(url)
                    return
                }
            }
            ...
            if let currentHash = currentCommit, !currentHash.isEmpty,
               let url = URL(string: "https://github.com/krishna2700/ghostty/compare/\(currentHash)...\(newHash)") {
                self = .compareTip(url)
            } else if let url = URL(string: "https://github.com/krishna2700/ghostty/commit/\(newHash)") {
                self = .commit(url)
            } else {
                return nil
            }
